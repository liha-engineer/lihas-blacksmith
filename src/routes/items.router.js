...
router.post('/items', async (req, res, next) => {
  ...
  if (item) return res.status(409).json({ message: '해당 아이템이 이미 존재합니다' });
  if (!itemCode) return res.status(400).json({ message: '아이템코드를 입력해주세요' });
  if (!price) return res.status(400).json({ message: '가격을 입력해주세요' });
  if (!type) return res.status(400).json({ message: '아이템 타입을 입력해주세요' });
